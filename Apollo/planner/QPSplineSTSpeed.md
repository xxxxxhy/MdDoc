...
## Define function for each spline segment
...
Given the assumption that the first point is $(t0, s0)$, and $s0$ is on the planned path $fi(t)$, $f′i(t)$, and $fi(t)″$ (position, velocity, acceleration). Apollo converts those constraint into QP equality constraints:
...
Sample m points on the path, for each $j$ and $j−1$ point pairs $(j∈[1,...,m])$:
...
Apollo first finds the lower boundary $l_{lb,j}$ at those points $(sj, lj)$ and $j∈[0,m]$ based on the road width and surrounding obstacles. Then it calculates the inequality constraints as:
...
Sample **m** points on the st curve, and get speed limits defined as an upper boundary and a lower boundary for each point $j$, e.g., $v_{ub,j}$ and $v_{lb,j} $. The constraints are defined as:
